...
h_old = np.array([0, 0, 0, 0, 0, 0], dtype=np.float32)
# for i in range(0,6):
#     h_old[i] = sim.data.qfrc_bias[i]
tau_old = np.array([0, 0, 0, 0, 0, 0], dtype=np.float32)
# for i in range(0,6):
#     tau_old[i] = sim.data.qfrc_bias[i]
# dfdfdfdf = tau_old
...
for step in range(duration):
    ...
    for i in range(max(0, len(ee_pos_buffer) - 900), len(ee_pos_buffer)-1):
        viewer.add_marker(pos=ee_pos_buffer[i],
                          ...
                          rgba=(1, 1, 0, 1))
    # if len(ee_pos_buffer) < 1000:
    #     for i in range(len(ee_pos_buffer)-100):
    #         viewer.add_marker(pos=ee_pos_buffer[i],
    #                           type=const.GEOM_SPHERE,
    #                           size=(0.01, 0.01, 0.01),
    #                           label='',
    ...
    # else:
    #     for i in range(900):
    #         viewer.add_marker(pos=ee_pos_buffer[np.floor(i * len(ee_pos_buffer) / 1000).astype('int')],
    #                           type=const.GEOM_SPHERE,
    #                           size=(0.01, 0.01, 0.01),
    #                           label='',
    #                           rgba=(1, 1, 0, 1))
